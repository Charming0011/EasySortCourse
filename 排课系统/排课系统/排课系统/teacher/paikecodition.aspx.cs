...
namespace 排课系统.teacher
{
    public partial class paikecodition : System.Web.UI.Page
    {
        ...
        public void bind()
        {
            string sqlstr = "select *,case shenhe  when '0' then '未审核' when '1' then '审核通过' else '审核未通过' end as shenhestatus from t_taboo where tabootype='0' and teachid='" + Session["teachid"].ToString() + "' order by id"; //1就是专业条件
        ...
        protected void Button1_Click1(object sender, EventArgs e)
        {
            ...
            if (Operation.getDatatable("select * from t_taboo where weekdays='" + DropDownList1.SelectedIndex.ToString() + "' and sections='" + DropDownList2.SelectedIndex.ToString() + "' and tabootype='0' and teachid='" + Session["teachid"].ToString() + "'").Rows.Count > 0)
            {
                WebMessageBox.Show("当前星期的当前节次已经禁忌排课，无需再次添加"); return;
            }
            string sql = "insert into t_taboo(weekdays,sections,weeksstart,weeksend,tabootype,teachid) values('" + DropDownList1.SelectedIndex.ToString()
                + "','" + DropDownList2.SelectedIndex.ToString() + "','" + DropDownList3.SelectedIndex.ToString() + "','" + DropDownList4.SelectedIndex.ToString() + "','0','" + Session["teachid"].ToString() + "')";
