...
{
    public partial class stuman : System.Web.UI.Page
    {
        ...
        public void bind()
        {
            string sqlstr = "select * from t_student where (stuname like '%" + this.findinfo.Text + "%' OR LEN('" + this.findinfo.Text + "')=0) order by stuid";
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            Operation.runSql("update t_student set stuname='" + ((TextBox)(GridView1.Rows[e.RowIndex].Cells[1].Controls[0])).Text.ToString().Trim() +
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            if (sheet != null)
            {
                string stuid, stuname, major, grade;
                //最后一列的标号
                ...
                for (int i = 1; i <= rowCount; ++i)
                {
                    ...
                    if (Operation.getDatatable("select * from t_student where stuid='" + row.GetCell(0).ToString().Trim() + "' or stuname='" + row.GetCell(1).ToString().Trim() + "'").Rows.Count > 0) continue;
                    stuid = row.GetCell(0).ToString().Trim(); stuname = row.GetCell(1).ToString().Trim(); major = ""; grade = "";
                    if (row.GetCell(2) != null) major = row.GetCell(2).ToString().Trim();
                    if (row.GetCell(3) != null) grade = row.GetCell(3).ToString().Trim();
                    string sql = "insert into t_student(stuid,stuname,major,grade,stupwd) values('" +
                    stuid + "','" + stuname + "','" + major + "','" + grade + "','" + stuid + "')";
                    Operation.runSql(sql);
                    count++;
                }
                WebMessageBox.Show("导入完成，成功导入数据记录共" + count + "条", "stuman.aspx");
            }
            else
            {
