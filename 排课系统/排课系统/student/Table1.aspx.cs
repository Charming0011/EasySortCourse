    ...
    public partial class Table1 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                Label1.Text = "欢迎您," + Session["stuname"].ToString() /*GetClassID()*/;
                //绑定
                bind();
            }
        }
        ...
        {
            string sql = "select classid from t_class1 where stuid='" + Session["stuid"].ToString()+"'";
        ...
        public void bind()
        {
            ...
            string sqlstr = "select weekdays,sections,coursename,t_coursetask.zhouci,major,grade,dianjiao,shuangyu " +
                            "from (t_coursetable left join t_coursetask on taskid=t_coursetask.id) left join t_class1 on classidz=classid where classid='" + GetClassID() + "'";
